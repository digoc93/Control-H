...
ControlH3.prototype.getSchedulesByUser= function(user,callback){
	Agenda.getAgendasByIdUser(user,function(err,result){
		if(err){
			...
			return callback(null, "The user don't have any registered shedule");
		}else{
			return callback(null,result);
		}
	});
...
ControlH3.prototype.getAllSchedules = function(callback){
	Agenda.getAgendasUsers(function(err,result){
		if(err){
			...
			return callback(null, "No registered shedules");
		}else{
			return callback(null,result);
		}
	});
...
ControlH3.prototype.getHoursInDateRange = function (info, callback){
	var fecha_fin= new Date(info.finalDate);
...
ControlH3.prototype.getAllProjects = function(callback){
	Project.getProjects(function(error,response){
		if(error){
			...
			return callback (null, "No registered Projects");
		}else{
			return callback (null, response);
		}
	});
...
ControlH3.prototype.getProjectById = function(idProject,callback){
	Project.getProjectById(idProject,function(error,response){
		if(error){
			...
			return callback(null,"The project requested does not exist");
		}else{
			return callback(null,response);
		}
	});
