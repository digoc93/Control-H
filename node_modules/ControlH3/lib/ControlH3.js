				...
				if(userResult.state == type){					
					...
					WorkSpace.finalisedWorkSpace(workSpace, function(error, result){
						if(error){
							...
							return callback("Could not find the active workspace");
						}
						else{
...
ControlH3.prototype.addProject = function (info, callback){
	Project.addProject(info,function(error,response){
		if(error){
			...
			Backlog.addBacklog({projectId: response._id, type: "product", editable : true}, function(err, result){
			...
			if(data.idUser != undefined){
				...
				User.getUserByQuery(query,function(errUser,res){
					if(errUser){
						...
						Requirement.addRequirement(idBacklog, data, function(error, response){
							if(error){
								return callback(error, null);
							}else if(!response){
			...
			if(data.idUser != undefined){
				...
				User.getUserByQuery(query,function(errUser,res){
					if(errUser){
						...
						//Generacion de un nuevo requierimiento, haciendo el viejo descartado
					}
				});
			}else{
